...
module.exports = {
    ...
    getStateVaccinationStatistic({ state, start_date, end_date }) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                latestStateDataFrame = datasets.query({
                    column: "state",
                    is: "==",
                    to: state
                });
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let datasets = await danfo.read_csv(
                    localVaccinationRegistrationCSV
                );
    ...
    getStateVaccinationRegistrationStatistic({ state, start_date, end_date }) {
        return new Promise(async (resolve, reject) => {
            try {
                let datasets = await danfo.read_csv(
                    stateVaccinationRegistrationCSV
                );
                let latestStateDataFrame;
                latestStateDataFrame = datasets.query({
                    column: "state",
    ...
    fetchLatestSummary() {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let population = await this.getPopulationByState({
                    state: "Malaysia"
                });
                ...
                for (let state of states) {
                    ...
                    let statePopulation = await this.getPopulationByState({
                        state
                    });
                    let stateReg = await this.getStateLatestVaccinationRegistrationStatistic(
                        { state }
                    );
                    responseData.push({
                        nme: state,
